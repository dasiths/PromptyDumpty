...
def test_validate_manifest_valid_package(tmp_path):
    ...
    manifest.write_text("""name: test-package
version: 1.0.0
...
def test_validate_manifest_invalid_group():
    """Test validate-manifest with invalid group for agent."""
    runner = CliRunner()
    result = runner.invoke(
        cli,
        ["validate-manifest", "tests/fixtures/invalid_group_package/dumpty.package.yaml"]
    )
    
    assert result.exit_code == 1
    assert "✗ Validation failed:" in result.output
    assert "Invalid artifact group 'invalid_group'" in result.output
    assert "copilot" in result.output
...
def test_validate_manifest_multiple_agents(tmp_path):
    """Test validate-manifest with multiple agents and groups."""
    manifest = tmp_path / "dumpty.package.yaml"
    manifest.write_text("""name: multi-agent-package
version: 1.0.0
...
def test_validate_manifest_copilot_invalid_rules_group(tmp_path):
    """Test that Copilot doesn't support 'rules' group."""
    manifest = tmp_path / "dumpty.package.yaml"
    manifest.write_text("""name: test-package
version: 1.0.0
description: Test package
...
def test_validate_manifest_nested_package():
    ...
    result = runner.invoke(
        cli,
        ["validate-manifest", "tests/fixtures/nested_package/dumpty.package.yaml"]
    )
